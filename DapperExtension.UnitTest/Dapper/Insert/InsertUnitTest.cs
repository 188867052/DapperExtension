...
namespace Core.UnitTest.Dapper.Insert
{
    ...
    public class InsertUnitTest
    {
        ...
        public void TestInsertWithSpecifiedPrimaryKey()
        {
            ...
            var id = DapperExtension.DapperExtension.Connection.InsertReturnKey(log);
            Assert.Greater(id, 0);
        }
        ...
        {
            var log = new Log { LogLevel = (int)LogLevel.Information, CreateTime = DateTime.Now, Message = "TestInsertWithSpecifiedPrimaryKey" };
            var idTask = DapperExtension.DapperExtension.Connection.InsertAsync(log);
            var id = await idTask;
            Assert.Greater(id, 0);
        }
        ...
        public void TestInsertWithMultiplePrimaryKeys()
        {
            ...
            string id = DapperExtension.DapperExtension.Connection.InsertReturnKey(keyMaster);
            Assert.IsNotNull(id);
        }
        ...
        {
            Task<dynamic> task = DapperExtension.DapperExtension.Connection.InsertAsync(new MultiplePrimaryKeyTable { Id = Guid.NewGuid().ToString("N"), Name = Guid.NewGuid().ToString("N") });
        ...
        {
            var log = new Log { LogLevel = (int)LogLevel.Information, Message = "TestInsertWithSpecifiedPrimaryKey" };
            dynamic task = await DapperExtension.DapperExtension.Connection.InsertAsync(log);
        ...
        public void TestMassInsert()
        {
            ...
            using (var transaction = DapperExtension.DapperExtension.BeginTransaction())
            {
                for (int i = 0; i < 2; i++)
                {
                    ...
                    count += DapperExtension.DapperExtension.Connection.Insert(log, transaction);
                }
